...
int main() {
    ...
    lazy_segtree<S, op, nn, F, mapping, composition, id> seg(n + 1);
    for (int i = 1; i <= n; ++i) {
        if (i > k) {
            seg.apply(tin[i - k], tout[i - k], -1);
        }
        seg.apply(tin[i], tout[i], 1);
        if (i >= k) {
            cout << seg.all_prod() << " ";
        }
    }
