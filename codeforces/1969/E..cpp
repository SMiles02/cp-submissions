...
using S = array<int, 2>;
S op(S a, S b) { return {a[0] + b[0], min(b[1], b[0] + a[1])}; }
S e() { return {0, (int)1e9}; }
...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        seg.set(i, {1, 1});
        if (prev[a[i]][1] != -1) {
            seg.set(prev[a[i]][1], {0, 0});
        }
        if (prev[a[i]][0] != -1) {
            seg.set(prev[a[i]][0], {-1, -1});
        }
        if (seg.prod(0, i + 1)[1] == 0) {
            if (prev[a[i]][1] != -1) {
                seg.set(prev[a[i]][1], {-1, -1});
            }
            if (prev[a[i]][0] != -1) {
                seg.set(prev[a[i]][0], {1, 1});
            }
